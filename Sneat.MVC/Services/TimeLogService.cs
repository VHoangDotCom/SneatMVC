...
namespace Sneat.MVC.Services
{
    public class TimeLogService
    {
        ...
        public async Task<JsonResultModel> UpdateTimeLog(TimeLogOutputModel input)
        {
            try
            {
                ...
                timeLog.TotalWorkingTime = input.TotalWorkingTime;
                timeLog.LogDate = input.LogDate;
                timeLog.UpdatedDate = DateTime.Now;

                await _dbContext.SaveChangesAsync();
                return JsonResponse.Success(SystemParam.MESSAGE_SUCCESS, timeLog);
            }
            catch (Exception ex)
            {
                return JsonResponse.Exception(ex.ToString());
